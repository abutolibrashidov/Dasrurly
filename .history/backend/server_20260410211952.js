...
// Simple users list for manager (added to satisfy frontend)
app.get('/api/users', authenticate, authorize(['manager']), (req, res) => {
  try {
    // Return users without passwords
    const safeUsers = (await Promise.resolve()).then(() => {
      return require('./src/config/database.js').users.map(u => ({ id: u.id, name: u.name, username: u.username, role: u.role }));
    });
    // safeUsers is a Promise due to require inside then - resolve
    Promise.resolve(safeUsers).then(list => res.json(list));
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
...
io.on('connection', (socket) => {
  ...
  // Attach authenticated user to socket if token passed via handshake
  try {
    ...
    // ignore - socket will be unauthenticated
  }

  // Request orders for kitchen (only authenticated kitchen/manager)
  socket.on('kitchen:requestOrders', () => {
    if (!socket.user || !['kitchen', 'manager'].includes(socket.user.role)) {
