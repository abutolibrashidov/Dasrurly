...
const socket = io('http://localhost:3000', { auth: { token: typeof localStorage !== 'undefined' ? localStorage.getItem('token') : null } });
...
function AppContent() {
  ...
  // If stored user exists but has no role (corrupt/old state), force re-login
  if (user && !user.role) {
    console.warn('Stored user has no role — forcing login');
    return <LoginPage />;
  }

  // Manager - show manager dashboard
  // Manager - show manager dashboard (accept both 'admin' and 'manager' roles)
  ...
  // Waiter or Kitchen - show legacy panels
  return (
    <div className="container">
      {!panel ? (
        <div className="panel-selector">
          <div className="selector-header">
            <h1 style={{ fontSize: '2.5em', marginBottom: '0.5em' }}>OSHXONA</h1>
            <p style={{ fontSize: '1em', margin: '0 0 1em 0', opacity: 0.9 }}>
  ...
  const handleSubmitOrder = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setMessage(`❌ ${err.error || 'Zakaz yuborilmadi'}`);
      }
    } catch (err) {
      console.error(err);
...
function KitchenPanel({ onBack }) {
  ...
  useEffect(() => {
    ...
    socket.on('orders:initial', handleInitialOrders);
